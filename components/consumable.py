...
class ConfusionConsumable(Consumable):
  ...
  def activate(self, action: actions.ItemAction) -> None:
    ...
    self.engine.message_log.add_message(f"{target.name} is confused!", color.status_effect_applied)
  ...
  def activate(self, action: actions.ItemAction) -> None:
    ...
    for actor in self.engine.game_map.actors:
      if actor.distance(*target_xy) <= self.radius:
        self.engine.message_log.add_message(f"The {actor.name} gets hit by the fireball, taking {self.damage} damage.")
        actor.fighter.take_damage(self.damage)
        targets_hit = True

    if not targets_hit:
      raise Impossible("No targets to hit in the fireball radius.")
